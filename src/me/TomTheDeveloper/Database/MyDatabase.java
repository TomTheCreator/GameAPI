    ...
    public MyDatabase(){
        config = ConfigurationManager.getConfig("database");
            ...
            if (!config.contains("IP"))
                config.set("IP", "<ip from database>");
            if (!config.contains("PORT"))
                config.set("PORT", 0000);
            if (!config.contains("DatabaseName"))
                config.set("DatabaseName", "FM");
            if (!config.contains("USER"))
                config.set("USER", "<Username>");
            if (!config.contains("PASSWORD"))
                config.set("PASSWORD", "<You're awesome password goes here>");
            if(!config.contains("Collection"))
                config.set("Collection", "FM");
            try {
